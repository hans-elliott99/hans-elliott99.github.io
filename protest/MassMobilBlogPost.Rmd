...
#
# display results in table with 'stargazer' package
...
## Recoding NAs
# NAs represent no prior protest
MMprotests$protesterviolence.lag1 <- MMprotests$protesterviolence.lag1 %>%
  replace_na(0)
...
```{r,fig.align='center'}
## Create grouped bar chart of state responses
...
```{r,echo=FALSE,results='asis'}
# Convert stateresponse variable to factor
...
#regress protester violence on type of state response
...
# Regress protester violence on lag of stateresponse
...
# Create dummy variables for each state response
...
# regress each state response type on protester violence and lag
...
# Create time series graph of state response types
...
# Use 'fixest' package to run regression with group fixed effects
p_load(fixest)
mod <- list()
...
# display results using 'modelsummary' package
p_load(modelsummary)
